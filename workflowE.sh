#!/bin/bash
echo "create view jointly as select workflow.batch_id, workflow.name, workflow.start_time, workflow.end_time, workflow.end_time-workflow.start_time as duration, workflow.status, workflow.message, workflow.data_status
...
while read wflows
        do
        echo "select name, batch_id from jointly where name = '$lines' order by batch_id desc limit 7;" | psql -d niara | sed '1,2d' | awk '{print $1,$3}' > failedbatches.txt
        done < failedWF.txt
